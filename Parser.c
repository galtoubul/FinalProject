...
Command parseCommand(char *input, int upperBound, MODE* mode){
    Command command;
    char* token;
    char *copyInput = (char*)malloc((strlen(input)+1));
    if(copyInput == NULL){
        printf(failedToAllocateMem,"parseCommand");
        exit(EXIT_FAILURE);
    }
    command.cmd = ERROR;
...
bool isLegalLengthCmd(char* str, int len){
    ...
    int i = 0;
    if(copyStr == NULL){
        printf(failedToAllocateMem,"isLegalLengthCmd");
        exit(EXIT_FAILURE);
    }
    strcpy(copyStr,str);
...
void copyFileAddress(char* str, Command* cmd){
    ...
    if(cmd->fileName == NULL){
        printf(failedToAllocateMem,"copyFileAddress");
        exit(EXIT_FAILURE);
    }
    while(str[0]){
